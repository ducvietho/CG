...
class UserController extends Controller
{
    ...
    public function getNoti(Request $request){
        $notis = Notification::where('user_to',Auth::id())->orderBy('created_at','DESC')->paginate();
        if($request->page == 1){
    ...
    public function getListRequested(Request $request){
        $user = User::find(Auth::id());
        $list = Care::where('type',$user->type)->where('status',2)->orderBy('created_at','DESC')->paginate();
